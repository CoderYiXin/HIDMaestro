...
|---|---|---|---|---|
| T22 | GameInputSvc prewarm (sc.exe spawn → cache hit) | n/a | ~15 ms (first only) | n/a |
| T23 | XInput slot-wait cadence 25 → 10 ms | n/a | ~15 ms (Xbox-family) | n/a |
| T24 | Cache devnode instId across UpdateDriverForPlugAndPlayDevices | n/a | ~1 ms | n/a |
| T25 | FinalizeNames cadence 100 → 25 ms + scaled budget | n/a | n/a | -50–75 ms (Phase 1.5) |
| T26 | Per-VID:PID OEM-write dedup in WriteInstanceConfig | n/a | ~few ms (dup VID:PID) | n/a |
| T27 | Skip GIP-buffer pack+copy for non-Xbox in SubmitState | ~60–80 instructions | n/a | n/a |
| T28 | WriteBits byte-aligned fast path | ~14 cycles per field × 6 fields = ~84 cycles | n/a | n/a |
| T29 | Parallelize HMContext ctor prewarm tasks | n/a | -50–100 ms (cold start) | n/a |
| T30 | Thread-local reusable buffer in GetHidChildId | n/a | -100 bytes/poll alloc | n/a |
| T31 | Skip GIP-buffer copy for non-Xbox in SubmitRawReport | -14 byte Marshal.Copy | n/a | n/a |
